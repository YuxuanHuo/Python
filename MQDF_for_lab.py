...
def QDF_model(train_data, class_num):  # Modified function from QDF to obtain the required parameters for MQDF
    ...
    for i in range(class_num):  # For all classes
        ...
        train_data[i] = np.matrix(train_data[i], np.float64) # Convert the ith data to a matrix & use 64-bit precision
        mean.append(np.mean(train_data[i],axis=0).T)  # Store the mean for all features in ith class
        # cov_matrix.append(np.cov(train_data[i].T))  # Store the covariance matrix of ith class，自己写
        cov_matrix.append(np.matrix(np.cov(train_data[i].T))) # Store the covariance matrix of ith class
        # Tip: use np.cov, consider a row as one feature. Pay attention to the data that may should be transposed
        ###############################################################################################################
    return mean, cov_matrix


def MQDF2_model(cov, d, k, class_num):  # Func00tion to obtain the trained parameters of MQDF2
    ...
    for i in range(class_num):  # For all classes
        ...
        eigenvalue.append(eigvals[:int(k)])  # Store the eigenvalues from j=1 to k
        eigenvector.append(eigvecs[:, 0:int(k)])  # Store the eigenvectors from j=1 to k

        delta[i] = sum(eigvals[int(k):]) / (d - k)  # Compute delta as the mean of minor values. Tip: sum(eigvals[int(k):]) / (d - k)
    ...
    for sample in np_x: # For the number of test samples
        ...
        for i in range(class_num): # For all classes
            ...
            for j in range(int(k)): # For the range of k
                ...
                g2 += (ma_dis[j] * 1.0 / eigenvalue[i][j]) + math.log(eigenvalue[i][j])
...
def MQDF1_model(cov, d, class_num, h):  # Function to obtain the trained parameters of MQDF1
    ...
    for i in range(class_num):  # For all classes
        ###############################################################################################################
        #                                   YOU NEED FILL FOLLOWING CODES:
        # covMat = cov[i] + h2I 要么covMat加上这个h2I，eigvalue不用加，要么cov不加这个，eigenvalue加h^2？
        covMat = cov[i] # Get the covariance matrix of ith class
        eigvals, eigvecs = np.linalg.eig(covMat) # Obtain eigenvalues and eigenvectors from the cov. matrix. Tip: usenp.linalg.eig()
        # id = eigvals.argsort()  # Get the ascending order of the eigenvalue indexes. Tip: use array.argsort()
        # id = id[::-1]  # Convert id to the descending order of the eigenvalues. Tip: use slicing as [::-1]
        # eigvals = eigvals[id]  # Descending the eigenvalues
        # eigvecs = eigvecs[id]  # Descending the eigenvectors 自己写
        ...
        # delta[i] = sum(eigvals) / d # Compute delta as the mean of minor values.
        ###############################################################################################################
    return eigenvalue, eigenvector, delta
    ...
    for sample in np_x:  # For the number of test samples
        ...
        for i in range(class_num):  # For all classes
            ...
            for j in range(int(d)):  # For the range of d
                ...
                g += (ma_dis[j] * 1.0 / eigenvalue[i][j]) + math.log(eigenvalue[i][j])
...
if __name__ == '__main__':
    ...
    ###############################################################################################################
    #                                   YOU NEED FILL FOLLOWING CODES:
    for h in range(100): # find a range of h for test
    ###############################################################################################################
        mqdf_sum_avg_acc = 0
        cnt = 1
        ...
        h_list.append(h*.01)
        mqdf_1_acc_list.append(MQDF_avg_acc)
    ...
    x1 = np.array(h_list)
    y1 = np.array(mqdf_1_acc_list)


    y1max = max(y1)
    print('The maximum accuracy of MQDF-1 is:',y1max)
    ...
    ##############################################################################################################
        ...
        k_list.append(k)
        mqdf_2_acc_list.append(MQDF_avg_acc)
    stop = timeit.default_timer() # Stop timer
    print('Running time of MQDF2 with 5-fold cross validation:', stop - start) # Running time of MQDF2

    x2 = np.array(k_list)
    y2 = np.array(mqdf_2_acc_list)


    y2max = max(y2)
    print('The maximum accuracy of MQDF-2 is:', y2max)
